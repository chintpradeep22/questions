...
def add_round_key(state, round_key):
    return [[s ^ k for s, k in zip(state_row, rk)] for state_row, rk in zip(state, round_key)]
...
def inv_mix_single_column(col):
    u = mul(col[0], 0x0e) ^ mul(col[1], 0x0b) ^ mul(col[2], 0x0d) ^ mul(col[3], 0x09)
    v = mul(col[0], 0x09) ^ mul(col[1], 0x0e) ^ mul(col[2], 0x0b) ^ mul(col[3], 0x0d)
    w = mul(col[0], 0x0d) ^ mul(col[1], 0x09) ^ mul(col[2], 0x0e) ^ mul(col[3], 0x0b)
    x = mul(col[0], 0x0b) ^ mul(col[1], 0x0d) ^ mul(col[2], 0x09) ^ mul(col[3], 0x0e)
    return [u, v, w, x]

def inv_mix_columns(state):
    return [inv_mix_single_column(col[:]) for col in state]
...
def encrypt_block(plaintext, key):
    ...
    for rnd in range(1, Nr):
        ...
        state = add_round_key(state, w[rnd * 4:(rnd + 1) * 4])
    state = sub_bytes(state)
    state = shift_rows(state)
